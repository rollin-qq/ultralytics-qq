...
# Constants
FILE = Path(__file__).resolve() #获取当前文件的绝对路径。另外：而os.getcwd()将返回当前工作目录的绝对路径，两者不同。 
ROOT = FILE.parents[2]  # 父目录向上移动两个层级，这里是ultralytics的目录
DEFAULT_CFG_PATH = ROOT / "yolo/cfg/default.yaml"
RANK = int(os.getenv('RANK', -1))
NUM_THREADS = min(8, max(1, os.cpu_count() - 1))  # number of YOLOv5 multiprocessing threads
#将环境变量中名为 'YOLO_AUTOINSTALL' 的值读取出来，并将其转换成小写字母形式的字符串，然后将其与字符串 'true' 进行比较。如果这两个字符串相等，则将 AUTOINSTALL 变量的值设置为 True；否则，AUTOINSTALL 变量的值将被设置为 False。
...
class IterableSimpleNamespace(SimpleNamespace):
    ...
    """
    #将该类实例的所有属性转换为一个字典，并返回该字典的迭代器。vars(self) 返回一个包含该实例所有属性的字典，其中键是属性名，值是属性值
    def __iter__(self):
        return iter(vars(self).items())

    #将该类实例的所有属性转换为一个字符串，每个属性以"属性名=属性值"的格式表示，并使用换行符连接起来。
    def __str__(self):
...
def yaml_save(file='data.yaml', data=None):
    ...
    """
    file = Path(file)   # 将文件路径转换为 Path 对象。
    ...
    with open(file, 'w') as f:
        ...
        #使用 yaml.safe_dump() 函数将数据字典以 YAML 格式写入到文件中。在写入之前，它通过遍历字典的键值对，将其中的 Path 对象转换为字符串。最终，该函数会关闭文件句柄。
...
def yaml_load(file='data.yaml', append_filename=False):
    ...
    """
    with open(file, errors='ignore', encoding='utf-8') as f:    #使用UTF-8编码方式读取文件内容，并将其存储在字符串变量f中
        # Add YAML filename to dict and return        
        s = f.read()  # string
        #如果字符串含有不可打印的特殊字符，则使用正则表达式从字符串中删除这些特殊字符。
        if not s.isprintable():  # remove special characters
            s = re.sub(r'[^\x09\x0A\x0D\x20-\x7E\x85\xA0-\uD7FF\uE000-\uFFFD\U00010000-\U0010ffff]+', '', s)
        #使用yaml.safe_load()方法解析YAML格式的字符串并将其转换为Python数据结构
        #最后，将Python数据结构与文件名一起存储在字典中返回，如果append_filename为False，则只返回Python数据结构。
